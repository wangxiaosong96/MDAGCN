...
class GraphSAINT(nn.Module):
    ...
    def forward(self, node_subgraph, adj_subgraph):   ##向前传播
        ...
        #np.savetxt("C:/Users/xs/Desktop/图采样有向图代码/test data/分类散点图数据/NMCMDA/二分类/feature.txt", emb_subg1 ,fmt='%s', newline='\n')
        #np.savetxt("C:/Users/xs/Desktop/图采样有向图代码/test data/分类散点图数据/TDRC/type1/feature.txt", emb_subg1 ,fmt='%s', newline='\n')
        
        
        
        #print('emb_subg',emb_subg.shape)
        #print('feat_subg',feat_subg.shape)
        #print('emb_subg',emb_subg.shape)
        #print('label_subg',label_subg) ##标签数据无变化
        #print('label_subg',label_subg.shape)
        #print('pred_subg',pred_subg)
        #print('emb_subg_norm',emb_subg_norm)
    ...
    def train_step(self, node_subgraph, adj_subgraph, norm_loss_subgraph):
        ...
        self.optimizer.step()
        #print('labels_converted',labels_converted.shape)
        #print('train_step_preds',preds.shape)
        #print('train_step_preds',preds)
        #print('train_labels:',labels)无变化
        #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/参数/train_step_preds1.txt", preds,fmt='%s', newline='\n')
        #preds.to_csv('C:/Users/Administrator/Desktop/图采样有向图代码/test data/参数/train_step_preds1.csv',index=False)
    ...
    def eval_step(self, node_subgraph, adj_subgraph, norm_loss_subgraph):
        ...
        with torch.no_grad():     #####可以让节点不进行求梯度，从而节省了内存控件，当神经网络较大且内存不够用时，就需要让梯度为False 直接输出flase  and  true
            ...
            #np.savetxt("C:/Users/xs/Desktop/图采样有向图代码/test data/分类散点图数据/NMCMDA/二分类/feature.txt", ys_test ,fmt='%s', newline='\n')
        #print('eval_labels:',labels)无变化    
        #print('test_label:',labels)
        #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/test_label.txt", labels,fmt='%s', newline='\n')
        return loss, self.predict(preds), labels
