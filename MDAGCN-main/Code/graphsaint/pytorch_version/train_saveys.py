...
def evaluate_full_batch(model, minibatch, mode='Test'):####Test  val
   ...
   ####输出
    ...
    #print('test_preds',preds)
    #print('test_preds',preds.shape)
    #print('Labels \n', labels)    #三列的标签矩阵
    #np.savetxt("C:/Users/Administrator/Desktop/图采有向图代码/test data/preds.txt", preds,fmt='%s', newline='\n')
    #print('Preds \n', len(preds))  ##3508
    if mode == 'val':
    ...
    #np.savetxt("C:/Users/xs/Desktop/图采样有向图代码/test data/分类散点图数据/NMCMDA/二分类/feature.txt", node_target1 ,fmt='%s', newline='\n')
    #np.savetxt("D:/2022年/case study/label[702].txt", node_target1 ,fmt='%s', newline='\n')
    ...
    for n in node_target:
        #print(labels[n,:].shape, preds[n,:].shape)    #[702, 3]
        #print(labels[n,1], preds[n,1])
        #print('labels[n,1]',labels[n,1])
        #print('labels[n,2]',labels[n,2])
        ...
        #print('labels.shape',labels.shape)  #[121624, 3]     [3508, 3]
        #print('labels.shape',labels[n].shape)# [24325, 3]     [702, 3]
        #print('preds.shape',preds.shape)
        #print('preds.shape',preds[n].shape)
        #ys, performances = metrics(to_numpy(labels[n, 0]), to_numpy(preds[n, 0]), model.sigmoid_loss)  #### metric.py代码
        #print('ys',ys)
    ...
    #print('ys:',ys)
    #print('ys(1):',to_numpy(labels[n, 1]))##标签只有0和1
    #print('labels[n].shape',labels[n].shape)
    #print('labels[n, 1]:',labels[n, 1])
    #np.savetxt("C:/Users/xs/Desktop/图采样有向图代码/test data/分类散点图数据/NMCMDA/二分类/label.txt", ys ,fmt='%s', newline='\n')
...
def train(train_phases, model, minibatch, minibatch_eval, model_eval, eval_val_every):
    ...
    for ip, phase in enumerate(train_phases):  #用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，同时列出数据和数据下标，一般用在 for 循环当中。
        ...
        for e in range(epoch_ph_start, int(phase['end'])):    ###epoch  设置为end=1000
            ...
            while not minibatch.end():
                ...
                #print('labels_train',len(labels_train))     ###2372,3
                #print('preds_train',preds_train)
                #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/参数/labels_train_all1.txt", labels_train,fmt='%s', newline='\n')
                #print('preds_train',preds_train)
                #print('to_numpy(labels_train[:, 1])',to_numpy(labels_train[:, 1]))
                if not minibatch.batch_num % args_global.eval_train_every:####  % 计算 a 除以 b 得出的余数。
                    
                    #ys_train, metrics_train = calc_metrics(to_numpy(labels_train[:, 1]),to_numpy(preds_train[:, 1]),model.sigmoid_loss, isprint = True)  
                    ys_train, metrics_train = metrics(to_numpy(labels_train[:, :]),to_numpy(preds_train[:, :]),model.sigmoid_loss, isprint = True)#, isprint = True
                    print('labels_train',labels_train.shape)
                    #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/labels_train1.txt", labels_train,fmt='%s', newline='\n')
                    #ys_train, metrics_train = metrics(to_numpy(labels_train[:, 0]),to_numpy(preds_train[:, 0]),model.sigmoid_loss, isprint = True)
    ...
    #print('loss_test:',loss_test) ##损失函数的值1.6074
    #print('ys_test:',ys_test)  验证预测的结果的值
    #print('metrics_test',metrics_test)   各个评价指标的参数
    #print('labels_train:',labels_train)
    printf("Total training time: {:6.2f} sec".format(time_train), style='red')
    #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/ys_test.txt", ys_test,fmt='%s', newline='\n')
    #np.savetxt("C:/Users/xs/Desktop/图采样有向图代码/test data/分类散点图数据/NMCMDA/二分类/label.txt", ys_test ,fmt='%s', newline='\n')###散点图
    #ys_train.to_csv('C:/Users/Administrator/Desktop/图采样有向图代码/test data/ys_train.csv',index=False)
...
if __name__ == '__main__':
    ...
    #print('ys_test',ys_test[2])
    #print('ys_test',ys_test[2].shape)
    #print('ys_test',type(ys_test))
    
    #np.savetxt("D:/2022年/case study/ys_test[0].txt", ys_test[0],fmt='%s', newline='\n')
    #np.savetxt("D:/2022年/case study/ys_test[1].txt", ys_test[1],fmt='%s', newline='\n')
    
    
    #np.savez_compressed('C:/Users/Administrator/Desktop/图采样有向图代码/test data/task_Tp_nobalanced_3knn_lr0.001_fold0——epoch-1.npz',  ys_test = ys_test)
    #np.savetxt("C:/Users/xs/Desktop/图采样有向图代码/test data/分类散点图数据/NMCMDA/三分类/ys_test[0].txt", ys_test[0],fmt='%s', newline='\n')
    #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/ys_test[1].txt", ys_test[1],fmt='%s', newline='\n')
    #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/ys_test[2].txt", ys_test[2],fmt='%s', newline='\n')
