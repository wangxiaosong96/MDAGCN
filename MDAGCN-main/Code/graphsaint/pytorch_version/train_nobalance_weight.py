...
def evaluate_full_batch(model, minibatch, mode='val'):
    ...
    for n in node_target:
        ...
        ys, performances = metrics(to_numpy(labels[n,:]), to_numpy(labels[n,:]), model.sigmoid_loss)
        ...
        for e in range(epoch_ph_start, int(phase['end'])):
            ...
            while not minibatch.end():
                ...
                if not minibatch.batch_num % args_global.eval_train_every:
                    ...
                    ys_train, metrics_train = metrics(to_numpy(labels_train[:, :]),to_numpy(labels_train[:, :]),model.sigmoid_loss, isprint = True)#, isprint = True
